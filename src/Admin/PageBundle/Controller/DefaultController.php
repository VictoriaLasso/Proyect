...
use Admin\PageBundle\Form\Type\FilterPagesFormType;
...
class DefaultController extends Controller {
    ...
    public function showPageAction() {
        ...
        if ($this->enableHTTPCache) {
            $response = $this->get('admin_page.services.http_cache_headers_handler')->setResponseCacheHeaders(null, $this->page->getDateLastModified(), false, 3600);
    ...
    protected function renderPage() {
        ...
        if ($this->enableHTTPCache) {
            $response = $this->get('admin_page.services.http_cache_headers_handler')->setResponseCacheHeaders(
                $response,
    ...
    public function filterPagesAction(Request $request) {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $filterTags = $this->get('admin_page.services.helpers')->getTagFilterTitles($filterData['tags']);
            $filterCategories = $this->get('admin_page.services.helpers')->getCategoryFilterTitles($filterData['categories']);
        }
    ...
    protected function renderHomePage() {
        ...
        $categoryIds = $this->get('admin_page.services.helpers')->getCategoryFilterIds($this->page->getCategories()->toArray());
        ...
        usort($pages, array("Admin\PageBundle\Controller\DefaultController", "sortHomepageItemsCompare"));
    ...
    protected function renderTagListPage() {
        ...
        $filterForm = $this->createForm(new FilterPagesFormType());
        $filterData = $this->get('admin_page.services.helpers')->getRequestedFilters($this->extraParams);
        $tagIds = $this->get('admin_page.services.helpers')->getTagFilterIds($filterData['tags']->toArray());
        $categoryIds = $this->get('admin_page.services.helpers')->getCategoryFilterIds($filterData['categories']->toArray());
    ...
    protected function renderCategoryPage() {
        $tagIds = $this->get('admin_page.services.helpers')->getTagFilterIds($this->page->getTags()->toArray());
        $categoryIds = $this->get('admin_page.services.helpers')->getCategoryFilterIds($this->page->getCategories()->toArray());
    ...
    protected function processContactForm(Request $request) {
        ...
        // Contact Form
        $contactForm = $this->container->get('admin_page.contact.form');
        $contactFormHandler = $this->container->get('admin_page.contact.form.handler');
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($contactFormProcess) {
                $formMessage = $this->container->get('translator')->trans('contact_form.response.success', array(), 'AdminPageBundle');
                $errorList = array();
                $formHasErrors = false;
            }
            else {
                $formMessage = $this->container->get('translator')->trans('contact_form.response.error', array(), 'AdminPageBundle');
                $errorList = $this->get('admin_page.services.helpers')->getFormErrorMessages($contactForm);
                $formHasErrors = true;
            }
    ...
    protected function onAjaxError($formHandler)
    {
        ...
        $ajaxFormData = array(
            'errors' => $errorList,
            'formMessage' => $this->container->get('translator')->trans($formMessage, array(), 'AdminPageBundle'),
    ...
    protected function sampleGuzzleCall(){
        ...
        try {
            ...
            $sampleAPIClientURL = "/repos/bardius/Admin/commits";
