...
namespace Admin\CommentBundle\Controller;
...
class DefaultController extends Controller {
    ...
    protected function createComment() {
        ...
        $formHandler = $this->get('admin_comment.comment.form.handler');
        ...
        if ($process) {
            ...
            $formMessage = $this->container->get('translator')->trans('comment.form.response.success', array(), 'AdminCommentBundle');
            $errorList = array();
            $formHasErrors = false;
        }
        else {
            $formMessage = $this->container->get('translator')->trans('comment.form.response.error', array(), 'AdminCommentBundle');
            $errorList = $this->get('admin_page.services.helpers')->getFormErrorMessages($form);
            $formHasErrors = true;
        }
        ...
        switch($this->commentType){
            ...
            default:
                return $this->get('admin_page.services.show_error_page')->errorPageAction(Page::ERROR_405);
        }
    }
    ...
    protected function getBlogPost() {
        ...
        $blogPost = $this->get('admin_settings.set_page_settings')->setPageSettings($blogPost);
    ...
    protected function returnAjaxResponse($errorList, $formMessage, $formHasErrors, $newComment) {
        $ajaxFormData = array(
            'errors' => $errorList,
            'formMessage' => $this->container->get('translator')->trans($formMessage, array(), 'AdminCommentBundle'),
