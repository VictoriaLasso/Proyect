...
use Admin\CommentBundle\Form\Type\CommentFormType;
use Admin\BlogBundle\Form\Type\FilterBlogPostsFormType;

use Admin\PageBundle\Entity\Page as Page;
use Admin\BlogBundle\Entity\Blog as Blog;
...
class DefaultController extends Controller {
    ...
    private function getResponseWithComments()
    {
        ...
        $form = $this->get('admin_comment.comment.form');

        // Retrieving the comments for the post
        $postComments = $this->get('admin_comment.services.commenthelper')->getBlogPostComments($this->id);
        ...
        if ('POST' === $this->pageRequest->getMethod() && $this->pageRequest->request->has('commentform_form')) {
            $response = $this->get('admin_comment.services.controller')->addCommentAction(Comment::TYPE_BLOG, $this->id, $this->pageRequest);
        } else {
            // Render normal page type
            $response = $this->render('BlogBundle:Default:page.html.twig', $pageParams);
        }
